...
export default function($q, Config, User) {
	...
	var checkStudentId = function(value) {
		var deferred = $q.defer();
		User.checkStudentNum({ student_num: [value] }).then(function(data) {
			var response = { result: true };
			if (_.has(data, 'length') && data.length) {
			...
			console.log('returning sid response: ', response);
			deferred.resolve(response);
		});
		return deferred.promise;
	};
	...
	validators['student id'] =
		new Validator('student id', { required: true, routine: checkStudentId, duplicates: true,
			regex: [Config.validation.studentId.regex] });
	...
	class GridValidator {
		...
		all(data) {
			...
			var studentIds = _.map(data, 'student id');

			var bulkValidation = [
			  User.checkStudentNum({ student_num: studentIds }),
			...
			$q.all(promiseArr).then(function(res) {
				$q.all(bulkValidation).then(function(res) {
					var studentsTaken = res[0];
					var usersTaken = res[1];
					// find indices of users and students who have been taken
					_.each(studentsTaken, function(student, i) {
						var index = _.findIndex(data, { 'student id' : student });
						if (index !== -1 && student !== '') {
							data[index].valid = false;
							self.matrix[index]['student id'] = { result: false, error: 'taken' };
						}
					});
