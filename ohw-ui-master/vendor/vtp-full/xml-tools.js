...
if (typeof window === "undefined") {
    var xmldom = require("./kb-xml-dom.js");
} else {
    xmldom = window;
}
...
function splitNode(node, pos) {
    ...
    var newNode = root.createElementNS(ns, node.tagName);
...
function splitTextNode(node, start, end, tag, tagClass) {
    ...
    var last = node.splitText(end);
    var mid = node.splitText(start);
    var newNode = root.createElementNS(ns, tag);
    newNode.textContent = mid.textContent;
    if (tagClass) {
        newNode.setAttribute("class", tagClass);
    }
    mid.parentNode.replaceChild(newNode, mid);
    return newNode;
}
...
function findContaining(node, text) {
    var out = [];
    if ((!node.childNodes || node.childNodes.length === 0) && node.textContent.indexOf(text) !== -1) {
        out.push(node);
    }
    if (node.childNodes) {
