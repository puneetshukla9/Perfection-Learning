...
export default function($filter, $sce, $q, Auth, $templateRequest, $rootScope,
		TableDefaults, $timeout, $compile, StudentUploadCSV, $state, StudentUploadValidation,
		StudentUploadGenerator, AppState, State, Student, School, User, $scope) {

		var schools = State.get('schools');
		$scope.initialized = false;

		var headerCellTemplate = require('./../templates/grid/student-upload-grid-header-cell.html');
		var editableCellTemplate = require('./../templates/grid/student-upload-editable-cell.html');
		var cellTemplate = require('./../templates/grid/student-upload-cell.html');
		var validCellTemplate = require('./../templates/grid/student-upload-cell-valid.html');

		var self = this;
		var file = $state.params.data;
		var students = StudentUploadCSV.convertToGridObject(file);
		...
		var config = {
			'first name': {
				...
				validator: StudentUploadValidation.validators.name.check,
				resolver: StudentUploadValidation.validators.name.resolve
			},
			'last name': {
				type: 'name',
				errors: 'Erase characters like !, @ and # from names.',
			...
			'student id': {
				...
				unique: true,
				autoResolve: false,
				canResolveEmpty: true,
				errors: 'Each student ID must be unique for students within your school. Check this cell for typos and make sure it\'s not already in use.',
				validator: StudentUploadValidation.validators['student id'].check,
				resolver: StudentUploadGenerator['student id']
			},
			'username': {
				type: 'username',
				// autoResolve: true,
				autoResolve: false,
				// refresh: ['first name', 'last name'],
				canResolveEmpty: true,
				errors:
					'Check to ensure the e-mail address you entered is valid. If not using an e-mail address as your username, ' +
					'please be sure that your username does not include any special characters (including =,<,>,+)',
				overwrite: $state.params && $state.params.changes && $state.params.changes.usernames,
				validator: StudentUploadValidation.validators.username.check,
				resolver: StudentUploadGenerator.username
			}
		};

		var tooltip = '';

		$scope.GridValidator = new StudentUploadValidation.GridValidator();
		...
		$scope.gridOptions = {
			...
			columnDefs: [
				...
				{ displayName: 'Last Name*',  field: 'last name', cellTemplate: cellTemplate,
					editableCellTemplate: editableCellTemplate, headerCellTemplate: headerCellTemplate },
				{ displayName: 'Student ID*', field: 'student id', cellTemplate: cellTemplate,
					editableCellTemplate: editableCellTemplate, headerCellTemplate: headerCellTemplate },
		...
		$scope.verifySchool = function() {
			...
			if ($scope.schoolFilter) {
				...
				schoolId = parseInt($scope.schoolFilter.id, 10); // Set school ID to add students to. schoolID used in buildUser.
		...
		$scope.submit = function() {
			...
			User.create(userData).then(function(res) {
				if (_.has(res, 'failure') && res.failure.length) {
					...
					$scope.saving = false;
					$scope.validRows = 0;
					$rootScope.$broadcast('notification error', { message: 'One or more of the students you submitted could not be created.' });
				} else {
					...
					$scope.saving = false;
					$scope.validRows = 0;
					$rootScope.$broadcast('notification confirmation', { message: 'Students successfully added!' });
				}
			}, function(err) {
				$scope.saving = false;
				$rootScope.$broadcast('notification error', { message: 'One or more of the students you submitted could not be created.' });
		...
		$scope.$on('grid correct count', function() {
			_.each($scope.gridOptions.data, function(row) {
				StudentUploadValidation.grid.row(row, $scope.gridOptions.data).then(function(data) {
					$scope.validRows = StudentUploadValidation.grid.count($scope.gridOptions.data);
				});
			});
		});
